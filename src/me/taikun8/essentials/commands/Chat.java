					...
					if(args.length == 1){
						if(args[0].equalsIgnoreCase("on")){
							if(ChatUtil.getChat() == false){
								ChatUtil.setChat("on");
								Bukkit.broadcastMessage(Main.getInst().getConfig().getString("chat-on"));
							}
						}
						if(args[0].equalsIgnoreCase("off")){
							if(ChatUtil.getChat() == true){
								ChatUtil.setChat("off");
								Bukkit.broadcastMessage(Main.getInst().getConfig().getString("chat-off"));
							}
						}
						if(args[0].equalsIgnoreCase("cc")){
							ChatUtil.clearChat();
							Bukkit.broadcastMessage(Main.getInst().getConfig().getString("chat-clear"));
							return false;
						}else{
							Util.sendMessage(p, "&cUzycie:&a /chat <cc/on/off>");
							return false;
